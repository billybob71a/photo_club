...
function usp_checkForPublicSubmission() {
	...
	if ($is_submitted) {
		...
		error_log('PeterY code: the user-submitted-image is'. print_r($files, TRUE));	
...
function usp_check_images($files, $newPost) {
	...
	if ($usp_options['usp_images'] == 'show') {
		...
		if (!empty($temp)) {
			...
			error_log('the values are '. $temp .' and '. $key .' and '. $value .' and '. $file_count);
		...
		for ($i = 0; $i < $file_count; $i++) {
			...
			error_log('under usp_check_images the file count is '. $file_count);
...
function usp_attach_images($post_id, $category, $newPost, $files, $file_count) {
	
	do_action('usp_files_before', $files);
	error_log('The file_count is '. $file_count);
	error_log('The file is '. $files);
	$attach_ids = array();
	...
	if ($files && $file_count > 0) {
		...
		for ($i = 0; $i < $file_count; $i++) {
			...
			if (isset($files['tmp_name'][$i]) && !empty($files['tmp_name'][$i])) {
				...
				error_log(' the tmp_name is going to be '. $tmp_name);
			...
			error_log('PeterY code: the array for $files is '. $files['name'][$i]);	
			if (isset($files['name'][$i]) && !empty($files['name'][$i])) {
				...
				$append = '-'.$post_id.'-'.$catstr;
				error_log(' The category is '. $category);

				error_log(' I am going to append '. $append);
				
				$append = apply_filters('usp_filename_append', $append);
				
				error_log('PeterY code: after append I apply filters and append'. $append);
			...
			error_log('PeterY code: the file name is '. $file_name);
...
function usp_createPublicSubmission($title, $files, $ip, $author, $url, $email, $tags, $captcha, $verify, $content, $category, $custom, $checkbox) {
	...
	error_log('the newPost id is '. $newPost['id']);
	...
	if ($post_id) {
		...
		$newPost = usp_attach_images($post_id, $category, $newPost, $files, $file_count);
